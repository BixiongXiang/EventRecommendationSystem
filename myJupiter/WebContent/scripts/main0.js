...
//this anonymous function will be executed on loading
(function() {

	// user information
	var user_id = '1111';
	var user_fullname = 'James';
	var lng = -122.08;
	var lat = 37.38;

	// initial event and elements
	function init() {
		// get nav button
	...
	/*
	 * below are helper functions, like load item
	 */

	/**
	 * helper function that activates the nav button
	 * 
	 * @param btnId:
	 *            id of nav button
	 */
	function activeBtn(btnId) {
		// we set the activated btn with class active
		var btns = document.querySelectorAll(".main-nav-btn");

		// deactivate all btn, by remove the class field using regex: \bactive\b
		for (var i = 0; i < btns.length; i++) {
			bens[i].className = btns[i].className.replace(/\bactive\b/, "");
		}
	...
	// show msg
	// replacing the item list content with msg in <p tag
	// give hint by using bootstrap icons
	...
	/**
	 * function that creats a DOM element format: <tag option ...>
	 * 
	 * @param tag
	 * @param options
	 * @returns {Element}
	 */
	function $cread(tag, options) {
	...
	function loadNearbyItems() {
		...
		var nearbyItems = mockSearchResponse; // mock will be replaced with
		// http response
	...
	// initialize after all parameter been loaded
	init();
