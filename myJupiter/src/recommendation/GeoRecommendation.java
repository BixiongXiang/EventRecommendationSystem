...
public class GeoRecommendation {
	public List<Item> recommendItems(String userId, double lat, double lon) {
		...
		// transfer to a set and sort using collection sort
		List<Entry<String, Integer>> categoryList = new ArrayList<>(allCategories.entrySet());
		
		Collections.sort(categoryList, (Entry<String, Integer> e1, Entry<String, Integer> e2) -> {
			return Integer.compare(e2.getValue(), e1.getValue()); // sort descendingly
		});
		
		// Step 3, search based on category, filter out favorite items
		Set<String> visitedItemIds = new HashSet<>(); //dedup
		for (Entry<String, Integer> category : categoryList) { 
			//search the category with bigger counts first
